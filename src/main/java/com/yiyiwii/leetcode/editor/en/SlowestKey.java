//A newly designed keypad was tested, where a tester pressed a sequence of n key
//s, one at a time. 
//
// You are given a string keysPressed of length n, where keysPressed[i] was the 
//ith key pressed in the testing sequence, and a sorted list releaseTimes, where r
//eleaseTimes[i] was the time the ith key was released. Both arrays are 0-indexed.
...
//
// The tester wants to know the key of the keypress that had the longest duratio
//n. The ith keypress had a duration of releaseTimes[i] - releaseTimes[i - 1], and
...
// 
//
// Return the key of the keypress that had the longest duration. If there are mu
//ltiple such keypresses, return the lexicographically largest key of the keypress
//es. 
//
// 
// Example 1: 
...
//Keypress for 'c' had a duration of 49 - 29 = 20 (pressed at time 29 right afte
//r the release of the previous character and released at time 49).
...
class Solution {
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (diff > maxDiff || (diff == maxDiff && keysPressed.charAt(i) > slowest)) {
                maxDiff = diff;
